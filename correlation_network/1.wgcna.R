...
pe_eigen = eigengene[12:22,]
blc_eigen = eigengene[23:34,]

de_ttest = list()
pe_ttest = list()
blc_ttest = list()
for(i in 1:16) {
  de_ttest[[i]] = t.test(de_eigen[1:6,i], de_eigen[7:11,i], alternative = "two.sided", var.equal = FALSE)
  pe_ttest[[i]] = t.test(pe_eigen[1:6,i], pe_eigen[7:11,i], alternative = "two.sided", var.equal = FALSE)
  blc_ttest[[i]] = t.test(blc_eigen[1:6,i], blc_eigen[7:12,i], alternative = "two.sided", var.equal = FALSE)
}

names(de_ttest) = names(pe_ttest) = names(blc_ttest) = colnames(mergedMEs)
holder = list(de = de_ttest,
              pe = pe_ttest,
              blc = blc_ttest)
...
cyan = list()
for(i in 1:3) {
  skyblue[[i]] = holder[[i]][["MEskyblue"]]
  brown4[[i]] = holder[[i]][["MEbrown4"]]
  turquoise[[i]] = holder[[i]][["MEturquoise"]]
  cyan[[i]] = holder[[i]][["MEcyan"]]
}
